...
class IndexNowSubmissionCommand extends Command
{
    ...
    public function handle(): int
    {
        ...
        $this->info("Submitting sitemap to search engines via IndexNow: {$sitemapUrl}");
        $this->newLine();
    ...
    protected function generateApiKey(): int
    {
        ...
        $this->info("Generated new IndexNow API key: {$key}");
        $this->newLine();

        $keyFileName = $this->indexNowService->getApiKeyFileName();
        $keyFileContent = $this->indexNowService->getApiKeyFileContent();

        $this->warn("IMPORTANT: You must create a file named '{$keyFileName}' in your website root.");
        $this->warn("The file should contain only this text: {$keyFileContent}");
        $this->warn('The file should be accessible at: ' . url($keyFileName));
